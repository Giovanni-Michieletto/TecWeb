<?php
    ...
    $page = file_get_contents('nuovoArticoloForm.html');
        ...
        if( strlen($Titolo)!=0 && strlen($Immagine)!=0 && strlen($AltImmagine)!=0 && strlen($Testo)!=0 ) {
            ...
            if($connection) {
                ...
                foreach ($listArticles as $article) {
                    if($Titolo == $article['Titolo']) {
                        $errorTitle = 'Titolo gia esistente';
                    }
                    if($imgContent == $article['Immagine']) {
                        $errorImage = 'Immagine gia esistente';
                    }
                    if($AltImmagine == $article['AltImmagine']) {
                        $errorAlt = 'AltImmagine gia esistente';
                    }
                    if($Testo == $article['Testo']) {
                ...
                if( strlen($errorTitle)==0 && strlen($errorImage)==0 && strlen($errorAlt)==0 && strlen($errorText)==0 ) {
                    ...
                    $insertion = $dbAccess->insertArticle($Titolo,$imgContent,$AltImmagine,$Testo);
                    ...
                    if($insertion == true) {
                        ...
                        $page = str_replace('action="nuovoArticolo.php"','action="nuovoArticoloForm.html"',$page);
                        $page = str_replace('>Inserisci<','>Nuovo form di inserimento<',$page);
                    }
                    else {
                        $message = '<div id="conferma"><p>Errore nell\'inserimento dell\'articolo</p></div>';
                    }
                }
            }
        }
